...
if(isset($_SESSION['id'],$_POST['formPrintFinal']) AND $_SESSION['id'] > 0) {
  ...
  switch ($typeOfMaterial) {
    case "raw_material" :
      $request = $bddmat->prepare('SELECT * FROM raw_material WHERE id = ?');
      ...
      Break;
    case "reagent" :
      $request = $bddmat->prepare('SELECT * FROM reagent WHERE id = ?');
      $request->execute(array($id));
        while ($result = $request->fetch()) {
      ...
      break;
    case "indicator" :
      $request = $bddmat->prepare('SELECT * FROM indicator WHERE id = ?');
      $request->execute(array($id));
        while ($result = $request->fetch()) {
      ...
      break;
    case "standard" :
      $request = $bddmat->prepare('SELECT * FROM standard WHERE id = ?');
      $request->execute(array($id));
        while ($result = $request->fetch()) {
      ...
      break;
    case "scale" :
      $request = $bddmat->prepare('SELECT * FROM scale WHERE id = ?');
      $request->execute(array($id));
        while ($result = $request->fetch()) {
